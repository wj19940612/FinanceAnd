...
public class FutureBattleRefactorActivity extends BaseActivity implements
    ...
    //登录账号是不是对战发起者
    private boolean mMineIsInitiator;
    //对战发起者订单
    ...
    private void showBattleOverCountdownTime() {
        ...
        Log.d(TAG, "showBattleOverCountdownTime: " + currentTime + " 开始 " + startTime + " " + mCurrentBattle.getEndTime());
        ...
        Client.getBattleInfo(mCurrentBattle.getId(), mCurrentBattle.getBatchCode()).setTag(TAG)
                .setCallback(new Callback2D<Resp<Battle>, Battle>() {
                    ...
                    protected void onRespSuccessData(Battle data) {
                        ...
                        Log.d("VolleyHttp", "超过5秒数据: " + data.toString());
                    }
    ...
    private void showGameOverDialog() {
        ...
        if (mCurrentBattle.getWinResult() == 0) {
            ...
            if (mCurrentBattle.getBattleResult(mMineIsInitiator)) {
    ...
    private void showRoomOvertimeDialog() {
        SmartDialog.single(getActivity(), getString(R.string.wait_to_overtime))
                ...
                .setNegative(R.string.recreate_room, new SmartDialog.OnClickListener() {
                    ...
                    public void onClick(Dialog dialog) {
                        dismissAllDialog();
                        Launcher.with(FutureBattleRefactorActivity.this, CreateBattleActivity.class).execute();
                        finish();
                    }
                })
    ...
    protected void onBattlePushReceived(WSPush<Battle> push) {
        ...
        if (mCurrentBattle != null && mCurrentBattle.isBattleOver()) { // OVER_PLAYERS + OVER_OBSERVER
            if (push.getContent() != null
                    && push.getContent().getType() == PushCode.BATTLE_JOINED) {
                Battle battle = (Battle) push.getContent().getData();
                showQuickJoinBattleDialog(battle);
            }
            return;
        }
        if (push.getContent() == null) return;
        Battle battle = (Battle) push.getContent().getData();
        switch (push.getContent().getType()) {
            ...
            case ORDER_CREATED:
                //对比订单房间操作次数 对不上就刷新
                if (battle != null) {
                    updateOrderStatus(battle, ORDER_CREATED);
                }
                break;
    ...
    private void updateCurrentOrder(TradeOrder order, int orderStatus) {
        ...
        TradeOrder mineOrder = mMineIsInitiator ? mInitiatorOrder : mAgainstOrder;
                ...
                .setCallback(new Callback<Resp<Battle>>() {
                    ...
                    public void onSuccess(Resp<Battle> battleResp) {
                        if (battleResp.getCode() == Battle.CODE_CREATE_FAST_MATCH_TIMEOUT) {
                            StartMatchDialog.dismiss(FutureBattleRefactorActivity.this);
                        }
                    }
...
//        }
    ...
    private void updateCurrentOrder(List<TradeOrder> data) {
        ...
        TradeOrder mMineTradeOrder = mMineIsInitiator ? mInitiatorOrder : mAgainstOrder;

        if (mMineTradeOrder != null) {
            setBattleTradeState(STATE_CLOSE_POSITION);
            if (mVariety != null) {
                mBattleTradeView.setTradeData(mMineTradeOrder.getDirection(),
                        FinanceUtil.formatWithScale(mMineTradeOrder.getOrderPrice(), mVariety.getPriceScale()), 0);
            } else {
                mBattleTradeView.setTradeData(mMineTradeOrder.getDirection(),
                        String.valueOf(mMineTradeOrder.getOrderPrice()), 0);
            }
        } else {
            setBattleTradeState(STATE_TRADE);
        }
...
//                startScheduleJob(0);
    ...
    public void onClosePositionButtonClick() {
        ...
        TradeOrder mMineTradeOrder = mMineIsInitiator ? mInitiatorOrder : mAgainstOrder;
        if (mMineTradeOrder != null) {
            closePosition(mMineTradeOrder.getId());
        }
    }
