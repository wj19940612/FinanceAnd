...
import com.sbai.finance.model.training.TrainingExperienceTotalCount;
...
public class TrainingDetailActivity extends BaseActivity {
    ...
    private void requestExperienceCount() {
        Client.getExperienceTotalCount(mTraining.getId()).setTag(TAG).setIndeterminate(this)
                .setCallback(new Callback2D<Resp<TrainingExperienceTotalCount>, TrainingExperienceTotalCount>() {
                    @Override
                    protected void onRespSuccessData(TrainingExperienceTotalCount data) {
                        mTrainingExperience.setSubText(getString(R.string.more_count, data.getMoreCount()));
                    }
                }).fire();
    ...
    private void requestShareData() {
        if (mTrainingDetail != null) {
            final String shareTitle = getString(R.string.train_share_share_title, mTrainingDetail.getTrain().getTitle());
            final String shareDescription = getString(R.string.train_share_description);
            final String shareUrl = String.format(Client.SHARE_URL_TRAIN_EXPERIENCE, mTraining.getId());
            Client.requestShareData(Share.SHARE_CODE_TRAINING)
                    .setIndeterminate(this)
                    .setTag(TAG)
